...
public class CFG {
    ...
    private void findUninitializedVars(){
        ...
        for (CFGBlock block: blocks){
            IRcommand command = block.getBody().get(0);
            if (command instanceof IRcommand_Load load){
                Variable loaded = block.getOut().get(load.var_name);
    ...
    private List<CFGBlock> constructGraph(List<IRcommand> commands) {
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            if (command instanceof IRcommand_Jumptype jumpCommand) { // FOR PROJECT: handle functions. label "endmain"?
    ...
    private void addVarOrTemp(IRcommand command, Set<String> nameSet) {
        String name = null;
        if (command instanceof IRcommand_Store store)
            name = store.var_name;
        else if (command instanceof IRcommand_Load load)
            name = TEMP_CHAR + load.dst.getSerialNumber();
        else if (command instanceof IRcommand_Binop binop)
            name = TEMP_CHAR + binop.dst.getSerialNumber();
        else if (command instanceof IRcommand_Allocate allocate)
