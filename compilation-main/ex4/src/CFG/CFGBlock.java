...
public class CFGBlock {
    ...
    private boolean recalcOutAfterIn(){
        ...
        if (command instanceof IRcommand_Binop binop) updateWithBinop(binop);
        else if (command instanceof IRcommand_Load load) updateWithLoad(load);
        else if (command instanceof IRcommand_Store store) updateWithStore(store);
        else if (command instanceof IRcommandConstInt constInt) updateWithConst(constInt);
        else if (command instanceof IRcommand_Allocate allocate) updateWithAlloc(allocate);

        return changed;
    }

    private boolean updateWithBinop(IRcommand_Binop binop){
        // this... feels bad... eh
    ...
    private boolean updateWithStore(IRcommand_Store store){
        String srcName = CFG.TEMP_CHAR + store.src.getSerialNumber();
        String varName = store.var_name;
    ...
    public boolean updateWithAlloc(IRcommand_Allocate alloc){
    ...
    public boolean updateWithLoad(IRcommand_Load load){
        // just a copy of store with a switch to the cfg
        String varName = load.var_name;
