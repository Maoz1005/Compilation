...
public class CFG {
    ...
    private List<List<CFGBlock>> constructGraph(List<IRcommand> commands) {
        ...
        Map<String, Integer> labelToIndex = findLabels(blocks);
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            if (command instanceof IRcommand_FuncDec funcDecCommand) {
    ...
    // Every sub grpah is representing a function (or the global scope)
    private List<CFGBlock> constructSubGraph(int startIndex, List<CFGBlock> blocks, boolean global, Map<String, Integer> labelToIndex) {
        ...
        // Well... is it necessary? prolly not?
        int i = startIndex;
        ...
        for (; i < blocks.size(); i++) {
            ...
            if (command instanceof IRcommand_Return returnCommand) {
                subGraph.add(block);
                block.addChild(demi); 
                demi.addParent(block);
                continue;
            }
            if (command instanceof IRcommand_Jumptype jumpCommand && !jumpCommand.ignoreCFG) {
            ...
            if (!(command instanceof IRcommand_Jump_Label unconditionalJump || // not while loop, backward edge
                i == blocks.size() - 1)) { // Last node
                if (blocks.get(i+1).getBody().get(0) instanceof IRcommand_FuncDec funcDecCommand) {
    ...
    private List<CFGBlock> getNodes(List<IRcommand> commands) {
        ...
        for (IRcommand command : commands) {
            // neri: no we really do want to add them in actually
            /*
            if ((command instanceof IRcommand_Jumptype jumpCommand) && (jumpCommand.ignoreCFG)) {
                // We don't want to add the jump command  with ignoreCFG flag to the CFG
                continue;
            }
            */
            ...
            curr.add(command);
            CFGBlock block = new CFGBlock(curr); // A list of single commands
            nodes.add(block);
        }
        return nodes;
    }
    ...
    private static Map<String, Integer> findLabels(List<CFGBlock> blocks) {
       Map<String, Integer> labelIndices = new HashMap<>();
        for (int i = 0; i < blocks.size(); i++) {
            IRcommand currentCommand = blocks.get(i).getBody().get(0);
            if (currentCommand instanceof IRcommand_Label curr) {
                labelIndices.put(curr.label_name, i);
            }
        }
        return labelIndices;
    }
